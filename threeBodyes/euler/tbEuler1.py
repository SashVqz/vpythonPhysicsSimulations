...
p = sphere(pos=vector(-3, 0, 0), radius=0.09, color=color.red, mass=0.1, make_trail=True, trail_type='points', interval=25, retain=15)
planets.append(p)
p = sphere(pos=vector(0, 0, 0), radius=0.09, color=color.blue, mass=massCentral, make_trail=True, trail_type='points', interval=25, retain=15)
planets.append(p)
p = sphere(pos=vector(3, 0, 0), radius=0.09, color=color.cyan, mass=0.1, make_trail=True, trail_type='points', interval=25, retain=15)
planets.append(p)

distance = mag(planets[0].pos - planets[1].pos)
v = np.sqrt(G * massCentral / distance)

planets[0].velocity = vector(0, v / 2, 0)
planets[1].velocity = vector(0, 0, 0)
planets[2].velocity = vector(0, -v / 2, 0)
